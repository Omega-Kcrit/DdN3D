...
public class PlayerController : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        switch (status)
        {
            case Status.sliding:
                SlideMovement();
                GameManager.Instance.ModifyHearthRate(0);
                break;
    ...
    void WallrunningMovement()
    {
        ...
        movement.Move(move, movement.runSpeed * 1.2f, (1f - s) + (s / 4f));
    }
    ...
    void UpdateLedgeGrabbing()
    {
        ...
        if (status != Status.climbingLedge)
        {
            ...
            if (status == Status.grabbedLedge)
            {
                ...
                if (down.y == -1)
                    status = Status.moving;
                else if (down.y == 1 || playerInput.Jump())
    ...
    private void OnTriggerEnter(Collider other)
    {
        switch (other.tag)
        {
            ...
            case "Checkpoint":
                GameManager.Instance.SetLastCheckpoint(other.transform);
